...
CzHCAPWallet::CzHCAPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhcap seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHCAPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHCAPSeed_deprecated()) {
                    LogPrintf("%s: Updated zHCAP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhcap seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhcap wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHCAPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHCAPCount(nCountLastUsed);
    else if (!walletdb.ReadZHCAPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHCAPWallet::Lock()
{
    seedMaster = 0;
}

void CzHCAPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHCAPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHCAP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHCAPWallet::LoadMintPoolFromDB()
{
...
void CzHCAPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHCAPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHCAPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhcapTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHCAPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHCAP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhcapTracker which also adds to database
    pwalletMain->zhcapTracker->Add(dMint, true);
...
void CzHCAPWallet::SeedToZHCAP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHCAPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHCAPWallet::GenerateDeterministicZHCAP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHCAPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHCAPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
