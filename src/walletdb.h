...
public:
    ...
    bool WriteZHCAPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHCAPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHCAPSeed_deprecated(uint256& seed);
    bool EraseZHCAPSeed();
    bool EraseZHCAPSeed_deprecated();

    bool WriteZHCAPCount(const uint32_t& nCount);
    bool ReadZHCAPCount(uint32_t& nCount);
