...
#include "ui_zhcapcontroldialog.h"
...
std::set<std::string> ZHcapControlDialog::setSelectedMints;
std::set<CMintMeta> ZHcapControlDialog::setMints;

ZHcapControlDialog::ZHcapControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHcapControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZHcapControlDialog::~ZHcapControlDialog()
{
    delete ui;
}

void ZHcapControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHcapControlDialog::updateList()
{
...
void ZHcapControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHcapControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZHcap_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHcapControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZHcapControlDialog::GetSelectedMints()
{
...
void ZHcapControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
