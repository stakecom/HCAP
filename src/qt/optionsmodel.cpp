...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeHealCapitalAmount"))
        settings.setValue("nAnonymizeHealCapitalAmount", 1000);

    nAnonymizeHealCapitalAmount = settings.value("nAnonymizeHealCapitalAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHealCapitalAmount"))
        SoftSetArg("-anonymizehealcapitalamount", settings.value("nAnonymizeHealCapitalAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in healcapital.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHealCapitalAmount:
            return QVariant(nAnonymizeHealCapitalAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHealCapitalAmount:
            nAnonymizeHealCapitalAmount = value.toInt();
            settings.setValue("nAnonymizeHealCapitalAmount", nAnonymizeHealCapitalAmount);
            emit anonymizeHealCapitalAmountChanged(nAnonymizeHealCapitalAmount);
            break;
        case CoinControlFeatures:
