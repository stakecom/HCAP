...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sHCAPPercentage, QString& szHCAPPercentage)
{
    int nPrecision = 2;
    ...
    szHCAPPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // HCAP labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, hcapAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelzHCAPPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zHCAP.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in healcapital.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in healcapital.conf");
    }
    ...
    bool showzHCAPAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzHCAPUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzHCAPImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzHCAPAvailable);
    ui->labelzBalanceMatureText->setVisible(showzHCAPAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzHCAPUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzHCAPUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzHCAPImmature);
    ui->labelzBalanceImmatureText->setVisible(showzHCAPImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelHCAPPercent->setVisible(showPercentages);
    ui->labelzHCAPPercent->setVisible(showPercentages);
