...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a HealCapital address (e.g. %1)").arg("MYAQChamL2LxZgWeNn5Mo8ggsVZcNXCPPc"));
#endif
...
void setupIPWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    // We don't want translators to use own addresses in translations
    // and this is the only place, where this address is supplied.
    widget->setPlaceholderText(QObject::tr("Enter a VPS IP (e.g. %1)").arg("127.0.0.2:20200"));
#endif
}
...
{
    // return if URI is not valid or is no HealCapital: URI
...
{
    // Convert healcapital:// to healcapital:
    //
    //    Cannot handle this later, because healcapital:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open healcapital.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HealCapital.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for HealCapital.lnk
...
{
    return GetAutostartDir() / "healcapital.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a healcapital.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=HealCapital\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the healcapital app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add healcapital app to startup item list
