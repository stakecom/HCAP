                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zHCAPpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zHCAP ought to be enough for anybody." - Bill Gates, 2017
    ui->zHCAPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzHCAPSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zHCAPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzHCAP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHCAP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zHCAP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zHCAP in ") +
...
void PrivacyDialog::on_pushButtonSpendzHCAP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHCAP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zHCAP
        sendzHCAP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zHCAP
    sendzHCAP();
}

void PrivacyDialog::on_pushButtonZHcapControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZHcapControlDialog* zHcapControl = new ZHcapControlDialog(this);
    zHcapControl->setModel(walletModel);
    zHcapControl->exec();
}

void PrivacyDialog::setZHcapControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzHCAPSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzHCAP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid HealCapital Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zHCAPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zHCAP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " HCAP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zHCAP</b>";
    ...
    // use mints from zHCAP selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZHcapControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZHcapControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zHCAP require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zHCAP"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZHCAP_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zHCAP require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zHCAP"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zHCAP was spent successfully update the addressbook with the label
    ...
    // Clear zhcap selector in case it was used
    ZHcapControlDialog::setSelectedMints.clear();
    ui->labelzHCAPSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHCAP Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zhcapTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zHCAP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zHCAP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zHCAP "));
    ui->labelzHCAPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzHCAPSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzHCAP->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zHCAP
        ui->pushButtonMintzHCAP->setEnabled(false);
        ui->pushButtonMintzHCAP->setToolTip(tr("zHCAP is currently disabled due to maintenance."));

        // Spend zHCAP
        ui->pushButtonSpendzHCAP->setEnabled(false);
        ui->pushButtonSpendzHCAP->setToolTip(tr("zHCAP is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zHCAP
        ui->pushButtonMintzHCAP->setEnabled(true);
        ui->pushButtonMintzHCAP->setToolTip(tr("PrivacyDialog", "Enter an amount of HCAP to convert to zHCAP", 0));

        // Spend zHCAP
        ui->pushButtonSpendzHCAP->setEnabled(true);
        ui->pushButtonSpendzHCAP->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}