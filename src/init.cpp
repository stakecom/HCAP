...
#include "config/healcapital-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("healcapital-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "healcapital.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "healcapitald.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the HCAP and zHCAP money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 20200, 22103));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, healcapital, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all HealCapital specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:20200"));
    ...
    strUsage += HelpMessageOpt("-backupzhcap=<n>", strprintf(_("Enable automatic wallet backups triggered after each zHcap minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zhcapbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zHCAP backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizehealcapitalamount=<n>     " + strprintf(_("Keep N HCAP anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 20201, 22104));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i HealCapital LLC"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("healcapital-loadblk");
...
/** Sanity checks
 *  Ensure that HealCapital is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. HealCapital Core is shutting down."));
...
#endif
    // Make sure only a single HealCapital process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. HealCapital Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("HealCapital version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //HealCapital specific: zerocoin and spork DB's
                ...
                // HealCapital: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZHCAPMinted();
                        RecalculateZHCAPSpent();
                    }
                ...
                // HealCapital: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of HealCapital Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart HealCapital Core to complete") << "\n";
        ...
        zwalletMain = new CzHCAPWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zHCAP wallet..."));

        bool fEnableZHcapBackups = GetBoolArg("-backupzhcap", true);
        pwalletMain->setZHcapAutoBackups(fEnableZHcapBackups);
...
//    }
//
//    nAnonymizeHealCapitalAmount = GetArg("-anonymizehealcapitalamount", 0);
//    if (nAnonymizeHealCapitalAmount > 999999) nAnonymizeHealCapitalAmount = 999999;
//    if (nAnonymizeHealCapitalAmount < 2) nAnonymizeHealCapitalAmount = 2;
    ...
    LogPrintf("Anonymize HealCapital Amount %d\n", nAnonymizeHealCapitalAmount);
    ...
    /* Denominations
       ...
       For example:
       1HCAP+1000 == (.1HCAP+100)*10
       10HCAP+10000 == (1HCAP+1000)*10
    */
