...
CzHCAPTracker::CzHCAPTracker(std::string strWalletFile)
{
...
CzHCAPTracker::~CzHCAPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzHCAPTracker::Init()
{
...
bool CzHCAPTracker::Archive(CMintMeta& meta)
{
...
bool CzHCAPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzHCAPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzHCAPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzHCAPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzHCAPTracker::GetSerialHashes()
{
...
CAmount CzHCAPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzHCAPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzHCAPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzHCAPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzHCAPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzHCAPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzHCAPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzHCAPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzHCAPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzHCAPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzHCAPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHCAPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHCAPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzHCAPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzHCAPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzHCAPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzHCAPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzhcap from DB\n", __func__, listDeterministicDB.size());
    }
