...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 HCAP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zHCAP send
enum ZerocoinSpendStatus {
    ZHCAP_SPEND_OKAY = 0,                            // No error
    ZHCAP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZHCAP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZHCAP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZHCAP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZHCAP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZHCAP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZHCAP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZHCAP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZHCAP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZHCAP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZHCAP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZHCAP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZHCAP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZHCAP_SPENT_USED_ZHCAP = 14,                      // Coin has already been spend
    ZHCAP_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZHCAP_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZHcapBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZHCAPOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzhcapAuto) const;
    ...
    */
    ...
    CzHCAPWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzHCAPTracker> zhcapTracker;
    ...
    void setZWallet(CzHCAPWallet* zwallet)
    {
        zwalletMain = zwallet;
        zhcapTracker = std::unique_ptr<CzHCAPTracker>(new CzHCAPTracker(strWalletFile));
    }

    CzHCAPWallet* getZWallet() { return zwalletMain; }
    ...
    void setZHcapAutoBackups(bool fEnabled)
    {
